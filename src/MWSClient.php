...
class MWSClient
{
    ...
    public function GetReportList($data = [] )
    {
        $array = [];
        $counter = 1;
        $ReportTypeList = !empty($data['ReportTypeList']) ? $data['ReportTypeList']: [];
        ...
        $array2 = [];
        $counter2 = 1;
        $ReportRequestIdList = !empty($data['ReportRequestIdList']) ? $data['ReportRequestIdList']: [];
        if(count($ReportRequestIdList)){
            foreach ($ReportRequestIdList as $ReportId) {
                $array2['ReportRequestIdList.Id.' . $counter2] = $ReportId;
                $counter2++;
            }
        }
     ...
     * @throws \GuzzleHttp\Exception\GuzzleException
     */
    public function RequestReport($report, $StartDate = null, $EndDate = null,$data = [])
    {
        $query = [
        ...
        if(!empty($data['ReportOptions'])){
            $query['ReportOptions'] = $data['ReportOptions'];
        }
        $result = $this->request(
            'RequestReport',
            $query
